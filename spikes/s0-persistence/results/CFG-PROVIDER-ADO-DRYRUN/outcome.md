# S0 Outcome: CFG-PROVIDER-ADO-DRYRUN

**Report date:** 2026-08-16
**Harness revision:** s0-harness-v2
**Configuration ID:** CFG-PROVIDER-ADO-DRYRUN
**Adapter:** ado
...
Executed 16 of 58 catalog scenarios (38 absolute gates not executed).
...
| Ownership marker | `tippani-s0:s0-provider-ado-dryrun` |
...
|---|---|---|---:|---|
| `S0-COL-002` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox with two test identities on one shared backing path. |
| `S0-COL-003` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox exercised from two devices/generations with reconnect. |
| `S0-COL-004` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox with lost-response fault injection against a real commit. |
| `S0-COL-005` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox with offline write then authoritative CAS confirmation. |
| `S0-COL-006` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox change-discovery feed observed by a second collaborator. |
| `S0-BCK-002` | absolute | Blocked | 0.000 | Blocked — requires an approved non-production OneDrive/SharePoint sandbox with ≥2 test identities for ETag/version precondition and version-restore tests. |
| `S0-BCK-003` | absolute | Blocked | 0.000 | Blocked — requires an approved disposable Azure DevOps sandbox repository and least-privilege identity for object/ref precondition and auditable-commit tests. |
| `S0-BCK-004` | absolute | Blocked | 0.000 | Blocked — requires an approved disposable GitHub sandbox repository and repository-scoped identity for object/ref precondition and auditable-commit tests. |
| `S0-BCK-005` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox with outage/throttle/auth-expiry/quota fault injection. |
| `S0-BCK-006` | relative | Blocked | 0.000 | Blocked — requires a Windows OneDrive sync-client profile to measure synced-folder conflicts. |
| `S0-BKP-003` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox version history/export to recover a known generation. |
| `S0-BKP-004` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox to prove a restored shared workspace has one authoritative head. |
| `S0-REC-003` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox with outage/auth/throttle/lost-response recovery. |
| `S0-REC-004` | absolute | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox plus a local offline cache to reconcile against newer authority. |
| `S0-MIG-004` | absolute | Blocked | 0.000 | Blocked — requires a live an approved live OneDrive/ADO/GitHub sandbox destination for local-to-shared rehome. |
| `S0-PER-004` | relative | Blocked | 0.000 | Blocked — requires an approved live OneDrive/ADO/GitHub sandbox to measure request count, bytes, throttling, and CAS latency. |
